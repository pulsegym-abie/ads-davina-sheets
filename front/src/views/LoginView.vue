  ...
  <div class="flex min-h-screen transition-colors duration-300">
    ...
    <div class="relative hidden w-1/2 lg:block xl:w-[55%]">
      <div class="absolute inset-0">
        <img src="/login-ads.png" alt="Banner" class="h-full w-full object-cover" />
      </div>
      <div class="absolute inset-0 bg-black/30" />
      <div class="relative flex h-full flex-col justify-end p-10 xl:p-14">
        <div class="max-w-lg">
          <p class="text-sm font-medium text-white/80 tracking-wide uppercase">{{ branding.tagline }}</p>
          ...
          <div class="mt-6 flex flex-wrap gap-x-6 gap-y-2">
            <div v-for="t in ['Meta Ads Analytics', 'Google Ads Analytics', 'Multi-Client Dashboard']" :key="t" class="flex items-center gap-2 text-sm text-white/90">
    ...
    <div class="flex w-full flex-col bg-background lg:w-1/2 xl:w-[45%]">
      ...
      <div class="flex flex-1 flex-col items-center justify-center px-6 pb-10 sm:px-12">
        <div class="w-full max-w-sm">
          <div class="mb-8">
            <div class="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-primary text-primary-foreground">
              <BarChart3 class="h-6 w-6" />
            </div>
            <h1 class="font-display text-2xl font-bold tracking-tight">Selamat Datang</h1>
            <p class="mt-1 text-sm text-muted-foreground">
              Masuk dengan akun Google Anda untuk melihat dashboard.
            </p>
          </div>
          ...
          <Button
            variant="outline"
            class="w-full rounded-xl h-12 gap-3 text-sm font-medium"
          ...
          >
            ...
            {{ loadingGoogle ? 'Menghubungkan…' : 'Login with Google' }}
          </Button>
